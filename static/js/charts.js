...
function buildCharts(sample) {
  ...
  d3.json("static/json/samples.json").then((data) => {
    ...
    // Hint: Get the the top 10 otu_ids and map (an action) them in descending order  
    //  so the otu_ids with the most bacteria are last. 

    var yticks = otuIds.slice(0,10).map(otuI => "OTU " + otuI).reverse();
    console.log(yticks);
